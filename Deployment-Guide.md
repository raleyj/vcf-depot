...
1. Obtain `VCF-Offline-Depot-1.0.0.ova`, its SHA-256 checksum, signature, release record, and release notes from the approved release location.
...
Record build **2026-09-05.2** and the supplied OVA SHA-256; the appliance version remains **1.0.0**, so the version label alone does not identify this rebuild.
...
4. Open the management GUI and use **Health Status** to check route, DNS, NTP, synchronization, and services.
...
| Time is not synchronized | NTP unreachable or its hostname cannot resolve | Verify DNS, routing, UDP 123 policy, and the NTP status in **Health Status** |
